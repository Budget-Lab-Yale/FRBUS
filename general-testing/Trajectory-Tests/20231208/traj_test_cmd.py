...
data = load_data("/gpfs/gibbs/project/sarin/shared/raw_data/FRBUS/v1/20231129/LONGBASE.TXT")
...
try:
        ...
        run = f"{count}, {end_long},{-1*delta_pers},{-1*delta_corp},-1,0,0,Failed while constructing baseline: {e} \n"
...
########################### STEP 3: Specify trajectory variables ########################## 
        ...
        try:
                ...
                run = f"{count}, {end_long},{-1*delta_pers},{-1*delta_corp},-1,0,0,Failed while simulating: {e} \n"
                #logging.error(error_message)
                with open('outcome_vars.csv', 'a') as f:
                        f.write(run)
        else:
                ...
                run = f"{count}, {end_long},{-1*delta_pers},{-1*delta_corp},1,{runtime},{last_surp},No Errors \n"
                with open('outcome_vars.csv', 'a') as f:
                        f.write(run)
